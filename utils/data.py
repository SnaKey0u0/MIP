...
import cv2
import numpy as np
import albumentations as albu   #　data augmetation
...
def read_data_file(data_fp):
    ...
    dirContent = os.listdir(data_fp)
    dirContent.sort()
    for i in range(0, len(dirContent)):
        image_path = os.path.join(data_fp, dirContent[i])
...
class Dataset(BaseDataset):
    def __init__(
        ...
        self.mask_fps = [x.replace('train', 'train_gt') for x in image_fps]
    ...
    def __getitem__(self, i):
        ...
        ignore_2 = np.all(mask == self.mask_values["polyp"], axis = -1).astype('float')
...
def get_train_augmentation(height, width):
    train_transform = [
        ...
        albu.ShiftScaleRotate(shift_limit = 0, scale_limit = 0,
                              rotate_limit = 10, border_mode = 0, p = 0.5), # shift_limit = 0.1, scale_limit = 0.1
    ...
    return albu.Compose(train_transform, p = 0.7)   # p = 1.0
...
def get_preprocessing(preprocessing_fn):    ######### 最好不要用
    _transform = [
...
def visualize(saved_path, **images):        ######### 還沒細看
    n = len(images)
