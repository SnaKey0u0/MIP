...
class Attention_block(nn.Module):
    def __init__(self, F_g, F_l, F_int):
        ...
        self.W_g = nn.Sequential(
            nn.Conv2d(F_g, F_int, kernel_size=1, stride=1, padding=0, bias=True), # 維度統一到F_int
            nn.BatchNorm2d(F_int)
        )

        self.W_x = nn.Sequential(
            nn.Conv2d(F_l, F_int, kernel_size=1, stride=1, padding=0, bias=True), # 維度統一到F_int
            nn.BatchNorm2d(F_int)
        )
    ...
    def forward(self, g, x):
        x0 = self.maxpool(x) # x必須下採樣至下一層
        g1 = self.W_g(g)
        x1 = self.W_x(x0)
        psi = self.relu(g1 + x1)
        psi = self.psi(psi)
        psi = self. up_samp(psi)
        return x * psi
